...
#HOST_CIS = '10.1.1.136:9400'
HOST_CIS = '192.168.1.6:9400'
API_URL_CIS = '/xml/AirPi/addFeed'
def do_request(xml_data):
        #print xml_data
        #print HOST_CIS
	webservice = httplib.HTTP(HOST_CIS)
        #print 'after httplin.host'
	webservice.putrequest("POST", API_URL_CIS)
	webservice.putheader("Host", HOST_CIS)
		...
		while(True):
			...
			if LOGGER:
				...
				try:
					...
					# print str(pac.geteeml())
					try:
						do_request(str(pac.geteeml()))
						print "Data uploaded to CIS" 
					except:
						print "Uploading data to CIS failed"
					pac.put()
					if stdscr == None:
						print "Uploaded data to Xively at " + str(datetime.datetime.now())
					GPIO.output(22, True)
					if LCDENABLED:		
